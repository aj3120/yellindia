...
class MenuAparels extends Component {
    ...
    onJumpSuits = () => {
        const show = this.state.show_jumpsuits === 'none' ? 'inline-block' : 'none';
        const hide = this.state.show_jumpsuits === 'none' ? 'none' : 'inline-block';
        this.setState({
    ...
    onShirts = () => {
        const show = this.state.show_shirts === 'none' ? 'inline-block' : 'none';
        const hide = this.state.show_shirts === 'none' ? 'none' : 'inline-block';
        this.setState({
    ...
    onShorts = () => {
        const show = this.state.show_shorts === 'none' ? 'inline-block' : 'none';
        const hide = this.state.show_shorts === 'none' ? 'none' : 'inline-block';
        this.setState({
            show_jumpsuits: 'none', show_jumpsuits_opposite: 'inline-block', show_shirts: 'none', show_shirts_opposite: 'inline-block',
    ...
    onTops = () => {
        const show = this.state.show_tops === 'none' ? 'inline-block' : 'none';
        const hide = this.state.show_tops === 'none' ? 'none' : 'inline-block';
        this.setState({
            show_jumpsuits: 'none', show_jumpsuits_opposite: 'inline-block', show_shirts: 'none', show_shirts_opposite: 'inline-block',
    ...
    showMenu = (event) => {
        const show = this.state.showMenuFlag === 'none' ? 'block' : 'none';
        const hide = this.state.showMenuFlag === 'none' ? 'none' : 'block';
    ...
    showApparels=()=>{
        const show = this.state.show_apparels === 'none' ? 'block' : 'none';
        this.setState({ show_apparels:show})
    }
    render() {
        const show_jumpsuits_style = this.state.show_jumpsuits === 'inline-block' ? { opacity: '1', height: '100%', transition: 'opacity 0.4s ease-in' } : { opacity: '0', height: '0px', transition: 'all 0.2s ease-out' }
        const show_shirts_style = this.state.show_shirts === 'inline-block' ? { opacity: '1', height: '100%', transition: 'opacity 0.4s ease-in' } : { opacity: '0', height: '0px', transition: 'all 0.2s ease-out' }
        const show_shorts_style = this.state.show_shorts === 'inline-block' ? { opacity: '1', height: '100%', transition: 'opacity 0.4s ease-in' } : { opacity: '0', height: '0px', transition: 'all 0.2s ease-out' }
        const show_tops_style = this.state.show_tops === 'inline-block' ? { opacity: '1', height: '100%', transition: 'opacity 0.4s ease-in' } : { opacity: '0', height: '0px', transition: 'all 0.2s ease-out' }
        return (
